...
public final class FourthIndicActiveConjugator extends RegularActiveConjugator {
	...
	protected void init() {
		...
		addRule(Tense.IMPERFECT, new PresentStrapStemRule(
				new AO2AmFixer(new ERule(new BaRule(new StemPlusPresentActiveEndingsRule())))));
		addRule(Tense.FUTURE,
				new PresentStrapStemRule(new EO2AMFixer(new ERule(new StemPlusPresentActiveEndingsRule()))));
