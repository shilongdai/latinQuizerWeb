...
public final class ThirdIOIndicActiveConjugator extends RegularActiveConjugator {
	...
	protected void init() {
		...
		addRule(Tense.IMPERFECT, new PresentStrapStemRule(
				new AO2AmFixer(new ConvertToIRule(new ERule(new BaRule(new StemPlusPresentActiveEndingsRule()))))));
		addRule(Tense.FUTURE, new PresentStrapStemRule(
				new EO2AMFixer(new ConvertToIRule(new ERule(new StemPlusPresentActiveEndingsRule())))));
